...
async def on_guild_join(guild):
    ...
    if (guild.id, ) not in serverList:
        cursor.execute(
            'INSERT INTO Servers (id, channel, options) VALUES (?, ?, ?)', (guild.id, -1, None))
        db.commit()
...
async def help(ctx, helpType=None):
    ...
    async def elaborate(embed):
        embed.add_field(
            name="Setup", value="Gives details on how to configure the bot in your server.")
        embed.add_field(
            name="Commands", value="A list of commands and a small example about how to use them.")
        await ctx.send(embed=embed)
...
async def channel(ctx, *, arg=''):
    ...
    if storedChannel is None and channelID != -1:
        ...
        chan = -1
        cursor.execute('UPDATE Servers SET channel=? WHERE id=?',
                       (chan, ctx.guild.id))
        db.commit()
...
async def feedback(ctx, *, arg=''):
    ...
    cursor.execute('SELECT id FROM bannedFeedback WHERE id=?',
                   (ctx.author.id,))
    if cursor.fetchone() is not None:
        cursor.execute(
            'SELECT reason FROM bannedFeedback WHERE id=?', (ctx.author.id,))
    ...
    if arg is None:
        ...
        return
    cursor.execute('INSERT INTO feedback (server, user, message) VALUES (?, ?, ?)', (int(
        ctx.guild.id), int(ctx.message.author.id), str(ctx.message.content)))
    db.commit()
...
async def banFeedback(ctx, id='', *, reason=''):
    ...
    if reason == '':
        ...
        return
    cursor.execute(
        'INSERT INTO bannedFeedback (id, reason) VALUES (?, ?)', (int(id), reason))
    db.commit()
    await send(ctx, 'Done!')
